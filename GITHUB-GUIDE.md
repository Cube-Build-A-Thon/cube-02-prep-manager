...
This repo is the official starting point for **02 · Prep Manager**.
...
1. [`README.md`](README.md) explains the Prep Manager problem, data and build expectations.
...
`https://github.com/Cube-Build-A-Thon/cube-02-prep-manager`
...
```sh
git clone git@github.com:<your-github-username>/cube-02-prep-manager.git
cd cube-02-prep-manager
```

### HTTPS

```sh
git clone https://github.com/<your-github-username>/cube-02-prep-manager.git
cd cube-02-prep-manager
```
...
```sh
git checkout -b feature/prep-verification
```
...
See [`README.md`](README.md) for the Prep Manager requirements.
...
git status
git add .
git commit -m "Implement prep verification"
...
```sh
git push -u origin feature/prep-verification
```
...
```text
Implement polybag verification
Add suffocation warning check
Add FNSKU label validation
Add expiry date check
Add handling mark detection
...
Before submitting, verify that your repository, documentation, demo, evaluation results and required links are final.

## 7. Asking for help, or reporting a problem

* **Something in the shared data or documentation is wrong or contradicts itself:** raise it with the organisers and identify it as a `finding`. Do not silently modify the official repository.
* **Access or GitHub permission problems:** make sure you are signed in to the GitHub account that owns your fork, or contact an organiser.
* **Questions about integration with other Managers:** follow the official Buildathon communication channels and evidence-contract guidance.
...
| Push rejected                   | Check that you are pushing to your own fork and the correct branch.                               |
...
If you do push a key by accident, **revoke the key immediately**. Removing the commit alone does not make an exposed credential safe.
